...
namespace MatFlat
{
    public static partial class Factorization
    {
        ...
        /// The matrix is not invertible.
        /// </exception>
        public static unsafe void LuInverse(int n, Complex* a, int lda, int* piv)
        {
            if (n <= 0)
            {
                throw new ArgumentException("The order of the matrix must be greater than or equal to one.", nameof(n));
            ...
            var work = ArrayPool<Complex>.Shared.Rent(n * n + n);
            try
            {
                fixed (Complex* pwork = work)
                {
                    ...
                    {
                        var aColj = a + lda * j;
                        var copySize = sizeof(Complex) * n;
                        Buffer.MemoryCopy(aColj, pwork + n * j, copySize, copySize);
                        diag[j] = aColj[j];
                        new Span<Complex>(aColj, n).Clear();
                    }

                    for (var j = 0; j < n; j++)
                    {
                        a[lda * piv[j] + j] = Complex.One;
                    }

                    for (var j = 0; j < n; j++)
                    {
                        pwork[n * j + j] = Complex.One;
                    }
                    for (var j = 0; j < n; j++)
                    {
                        var aColj = a + lda * j;
